...
void SceneManager::Init()
{
	...
	for(int  i  = 0; i< num;)
	{
		...
		if(temps == "RECT")
		{
			...
			float *tempRectInfor = new float[5];
			
			fscanf(file, "POSITION: %f, %f, %f, %f\n",(tempRectInfor), (tempRectInfor+1), (tempRectInfor+2), (tempRectInfor+3));
			fscanf(file, "VELOCITY: %f\n\n", (tempRectInfor +4));
			Rectangles *temprect = new Rectangles(tempRectInfor[0], tempRectInfor[1],1.0,tempRectInfor[4], tempRectInfor[2], tempRectInfor[3]);
			AddObject(temprect);
		...
		{
			float *tempCirInfor = new float[4];
...
void SceneManager::touchActionUp(int x, int y)
{
	...
	for (int i = 0; i < m_Objects.size() -1; i++)
	{
		if (m_Objects.at(i)->mouseMovable() && m_deltaX && m_deltaY )
		{
			...
			m_Objects.at(i)->resetGravityAble();
			
		}
		else if (m_deltaX == 0 && m_deltaY == 0)
		{
